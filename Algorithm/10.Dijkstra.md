...
이때 다익스트라는 간선의 가중치가 음수가 아니라는 전제가 있으므로, 해당 시점에 선택된 정점의 거리는 이후 어떤 경로를 거쳐도 더 짧아질 수 없어 최단 거리로 즉시 확정될 수 있다.<br>
...
따라서 우선순위 큐에서 꺼낸 값이 최신화된 최단 거리인지를 확인하는 필터링 절차를 반드시 거쳐야지만 불필요한 연산을 방지하고 우선순위 큐의 효율을 제대로 얻을 수 있다.
...
int main()
{
  ...
  while (!pq.empty())
  {
    ...
    if (dist[current_node] != current_dist) // 큐에서 꺼낸 값이 최신화된 최단 거리인지를 확인하는 절차
      continue;
