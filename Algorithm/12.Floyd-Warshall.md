...
플로이드–워셜은 중간에 경유할 수 있는 노드 집합을 {1}, {1, 2}, ..., {1...k} 처럼 단계적으로 확장해 가며 최단 거리를 확정하는 DP 방식으로 동작한다.<br>
...
int main()
{
  ...
  for (int i = 0; i < m; i++)
  {
    ...
    // 플로이드-워셜을 구현할 때는 인접 리스트(graph) 형태로 처리하기보다,
    // dist[i][j] 형태의 "거리 행렬"을 사용한다.
    // 따라서 입력값에 같은 (a → b) 간선이 여러 개 주어질 수 있으므로,
    // 그중 가장 비용이 작은 간선만 dist[a][b]에 반영하기 위한 코드이다.
    // 인접 리스트에서는 같은 간선이 주어져도 둘 다 저장되기 때문에 굳이 필요하지 않다.
    // * 주의! (long long, int)와 같이 두 대상의 타입이 다르면 비교 불가
  }
