...
\* 주파수 분할 이중 방식(FDD, Frequency Division Duplex):<br>
상향(Uplink, 단말(휴대폰) → 기지국)과 하향(Downlink, 기지국 → 단말(휴대폰)) 통신에 서로 다른 주파수 대역을 할당하여, 같은 시간에 동시에 송수신이 가능하게 하는 이중화 방식
...
반면, 링형 토폴로지에서는 토큰을 가진 노드만 전송하고 다른 노드들은 전송 대기 상태에 있어, 충돌이 발생하지 않는다.(토큰 패싱)
...
\* 패킷 교환의 방식으로는 크게 연결형인 가상 회선 방식과 비연결형인 데이터그램 방식 두 가지가 있다.

### 메세지 교환

전송 데이터를 메시지 단위(전송하려는 전체 데이터 단위)로 전달하는 방식이다.<br>
각 노드는 메시지를 완전히 수신하여 완성본을 확보한 뒤 저장(store)하고, 이후 다음 노드로 전달(forward)하는 Store-and-Forward 구조를 가진다.

장점: 전용 회선 독점 없이 전달 가능(자원 공유)<br>
